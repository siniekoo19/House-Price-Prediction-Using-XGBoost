...
waterfront_ = st.selectbox("Select do you want waterfront", ['yes', 'no'])
if waterfront_ == 'yes':
    waterfront_ = 1
else:
    waterfront_ = 0
...
condition_ = st.selectbox("Select in what type of conidition you want", ['Excellent', 'Above Average', 'Average', 'Below Average', 'Poor'])
...
sqft_basement_ =  st.number_input("Select a square footage of the basement", df['sqft_basement'].min(), df['sqft_basement'].max()) 

yr_built_ = st.number_input("Which year do you want the house should built", df['yr_built'].min(), df['yr_built'].max())
...
is_renovated_ = st.selectbox("Are you want a renovated house", ['yes', 'no'])
...
if st.button(f"**Predict Price**"):
    # Feature Scaling using Standard Scaler Model
    df_new = sc.transform(df_new)
    
    # Predicting using the loaded XGBoost model
    y_prediction = xg_loaded.predict(df_new)
    
    # Display the predicted price
    st.markdown(f"## :blue[Price of the house is :] <span style='font-size:36px'>{y_prediction[0]}</span>", unsafe_allow_html=True)